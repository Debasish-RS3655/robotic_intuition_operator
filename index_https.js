...
let LLM = null; // keep same global symbol name so rest of code doesn't change

function initLLMApi() {
  try {
    const apiKey = process.env.GROQ_API_KEY;
    if (!apiKey) { console.warn('GROQ_API_KEY missing; LLM phrasing disabled.'); return; }
    const client = new Groq({ apiKey });
    // unify shape with old LLM object so llmPhrase can stay the same call site
    LLM = { client, model: (process.env.GROQ_MODEL || 'llama3-8b-8192') };
    console.log('✅ Groq LLM client ready.');
  } catch (e) {
...
function toneFromEmotions(e) {
  // pick dominant emotion and intensity words
  const entries = Object.entries(e).sort((a, b) => b[1] - a[1]);
  const [name, val] = entries[0] || ['neutral', 0];
  const intensity = val > 0.75 ? 'strong' : val > 0.5 ? 'moderate' : 'subtle';
  const tones = {
    joy: ['warm', 'encouraging', 'upbeat'],
    sadness: ['gentle', 'empathetic', 'soft-spoken'],
    anger: ['firm', 'concise', 'no-nonsense'],
    fear: ['cautious', 'reassuring', 'calm'],
    disgust: ['detached', 'matter-of-fact', 'reserved'],
    surprise: ['curious', 'lively', 'engaged'],
    neutral: ['balanced', 'polite', 'clear']
  };
  const arr = tones[name] || tones.neutral;
  return { name, intensity, style: arr.join(', ') };
}

async function llmPhrase({ userText, emotions, decision, chosenSurface, voiceHints }) {
  if (!LLM?.client) return null;

  // added a tiny guard for cost/rate limits
  if (!userText || userText.trim().length < 2) return null;


  const tone = toneFromEmotions(emotions);
  const decisionDirective = (() => {
    if (!decision) return 'No explicit decision required. Respond naturally.';
    switch (decision.type) {
      case 'action_liked': return `Convey willingness to proceed.`;
      case 'action_not_liked': return `Politely decline doing it.`;
      case 'action_unknown': return `Admit uncertainty and ask one clarifying question.`;
      case 'single_option_type':
        return decision.subtype === 'option_liked'
          ? `State preference for "${chosenSurface || 'the chosen option'}".`
          : decision.subtype === 'option_not_selected'
            ? `Politely reject "${chosenSurface || 'the option'}".`
            : `Explain you don't know "${chosenSurface || 'the option'}".`;
      case 'multi_option_type':
        if (decision.subtype === 'options_unknown') return 'Explain none of the options are known.';
        if (decision.subtype === 'uncertain_state') return 'Explain you are torn; ask one short follow-up.';
        if (decision.subtype === 'options_not_selected') return 'Say none appeal right now.';
        return `State preference for "${chosenSurface || decision.subtype}".`;
      default: return 'Respond naturally.';
    }
  })();
...
Do not choose options; choices are already decided. Just phrase the response.`;
  ...
  const instr = `Instruction: ${decisionDirective}`;
  const usr = `User: ${userText}`;
  ...
  try {
    const resp = await LLM.client.chat.completions.create({
      model: LLM.model,              // e.g. llama3-8b-8192
      messages,
      temperature: 0.7,
      max_tokens: 80,
      stream: false
    ...
    console.warn('LLM API phrasing error:', e.message);
    return null;
  }
}
...
function startOnce() {
  ...
  initLLMApi();      // Initialize LLM (if applicable)
...
app.post("/output", async (req, res) => {
  try {
    ...
    if (b.recognisedSpeech?.data) {
      ...
      // add this:
      const decisionProbe = decisionSpeechExtractor.decisionSpeech(text);
      const IS_DECISION_SPEECH = !!decisionProbe;
      ...
      if (willSpeak) queueSpeech += finalReply + " ";


      let stimuli = [];
      if (!IS_DECISION_SPEECH) {
        // only do emotion analysis + memory update for NON-decision utterances
        stimuli = await tokenStimuliFromText(text, personsPresent[speakerIdx].personality);
        ...
        // Decision turn → skip emotion analysis + memory writes
        if (testMode) console.log('Decision turn: skipping token emotion analysis and memory updates.');
      }
...
https.createServer(options, app).listen(3000, () => {
  console.log('HTTPS server running at https://raspberrypi.local:3000');
